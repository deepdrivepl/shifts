...
def ensemble_uncertainties_classification_pytorch(probs, epsilon=1e-10):
    ...
    conf = torch.max(mean_probs, axis=-1)

    eoe = entropy_of_expected_pytorch(probs, epsilon)
    exe = expected_entropy_pytorch(probs, epsilon)

    mutual_info = eoe - exe

    epkl = -torch.sum(mean_probs * mean_lprobs, axis=-1) - exe
