...
PARAMS = dict(
    ...
    # trainining
    n_epochs=80,
    accumulated_batch_size=24,
    batch_size=6,
    
    optimizer=torch.optim.RAdam,
    optimizer_params=dict(lr=1e-3),
    scheduler=torch.optim.lr_scheduler.OneCycleLR,
    scheduler_params=dict(max_lr=3e-3, div_factor=30, final_div_factor=100, pct_start=0.1),
    monitor=None,
...
def loss_function(outputs, labels):
    ...
    class_weights=torch.tensor([100,1], dtype=torch.float).to(outputs.device)
