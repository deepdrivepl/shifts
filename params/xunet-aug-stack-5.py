...
from monai.transforms import (
    AddChanneld, EnsureChannelFirstd, Compose, LoadImaged, RandCropByPosNegLabeld,
    ...
    RandGaussianSharpend, RandGaussianSmoothd, RandGibbsNoised, RandAdjustContrastd,
    RandBiasFieldd, Rand3DElasticd)
...
PARAMS = dict(
    ...
    scheduler_params=dict(max_lr=1e-3, div_factor=10, final_div_factor=30, pct_start=0.02),
    monitor=None,
...
def get_train_transforms():
    return Compose(
        [
            ...
            # AddChanneld(keys=["image", "label"]), #deprecated
            EnsureChannelFirstd(keys=["image", "label"]),
            # NormalizeIntensityd(keys=["image"], nonzero=True),
            ScaleIntensityd(keys="image"),
            RandShiftIntensityd(keys="image", offsets=0.1, prob=0.5), # brightness adjustment
            RandScaleIntensityd(keys="image", factors=0.1, prob=0.5), # perturbation
            ...
            RandAffined(keys=['image', 'label'], mode=('bilinear', 'nearest'),
                        prob=0.5, spatial_size=(64, 64, 64),
                        rotate_range=((np.pi / 9, np.pi / 9), (np.pi / 9, np.pi / 9), (np.pi / 9, np.pi / 9)),
                        scale_range=((0.4, 1.6), (0.4, 1.6), (0.4,1.6)), padding_mode='border'),
            RandGaussianSharpend(keys="image", prob=0.5),
            RandGaussianSmoothd(keys="image", prob=0.5),
            RandGibbsNoised(keys="image", alpha=(0.1, 1.0), prob=0.5),
            RandAdjustContrastd(keys="image", prob=0.5),
            RandBiasFieldd(keys="image", prob=0.5),
            Rand3DElasticd(keys=["image", "label"], prob=0.5, sigma_range=(10,13), magnitude_range=(50,150), padding_mode='zeros',
...
def get_val_transforms(keys=["image", "label"], image_keys=["image"]):
    ...
    return Compose(
        [
            ...
            # AddChanneld(keys=keys), # deprecated
