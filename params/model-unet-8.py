...
def loss_function(outputs, labels):
    ...
    dice_ce_fn = DiceCELoss(to_onehot_y=True, 
                       softmax=True, sigmoid=False,
                       include_background=False)
    dice_ce_loss = dice_ce_fn(outputs, labels)


    return {'loss': dice_ce_loss, 'dice_loss': dice_loss.detach().cpu(), 'focal_loss': focal_loss.detach().cpu()}
