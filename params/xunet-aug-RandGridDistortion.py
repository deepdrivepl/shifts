...
from monai.transforms import (
    ...
    RandRotate90d, RandShiftIntensityd, RandAffined, RandSpatialCropd, RandCoarseDropoutd,
    SavitzkyGolaySmoothd, RandAdjustContrastd, RandGibbsNoised,
    RandBiasFieldd, Rand3DElasticd, RandGridDistortiond,
...
PARAMS = dict(
    ...
    # logging
    tb_logs='./runs/aug',
    exp_name='xunet-aug-RandGridDistortiond',
...
def get_train_transforms():
    return Compose(
        [
            ...
            # RandCoarseDropoutd(keys=["image", "label"], holes=16, spatial_size=16, prob=0.5),
            # SavitzkyGolaySmoothd(keys="image", window_length=7, order=3),
            # RandAdjustContrastd(keys="image", prob=0.5, gamma=(0.5, 4.5)),   # x = ((x - min) / intensity_range) ^ gamma * intensity_range + min
            # RandGibbsNoised(keys="image", prob=0.5),
            # RandBiasFieldd(keys="image", prob=0.5),
            # Rand3DElasticd(keys=["image", "label"], sigma_range=(5,7), magnitude_range=(50,150), prob=0.5, padding_mode='zeros',
            #                mode=["bilinear", "nearest"]),
            RandGridDistortiond(keys=["image", "label"], prob=0.5),
